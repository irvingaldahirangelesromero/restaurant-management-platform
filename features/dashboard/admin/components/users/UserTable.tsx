...
export function UserTable({
  ...
  const getInitials = (u: StaffMember) => (u.name[0] + u.lastname[0]).toUpperCase();
        ...
        <table className="w-full border-collapse">
          ...
          <tbody className="divide-y divide-border/60">
            {users.length === 0 ? (
              ...
              users.map((u) => {
                const areaCls = AREA_COLORS[u.area] || "bg-gray-50 text-gray-600 border-gray-100";
                  ...
                  >
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-2 text-[10px] font-black text-text-muted uppercase tracking-wider">
                            <span className="bg-surface border border-border px-1.5 py-0.5 rounded-md shadow-sm">{u.id}</span>
                            <span>•</span>
                    ...
                    <td className="px-6 py-5">
                       <div className="space-y-1.5">
                          <span className={`inline-flex px-2.5 py-1 rounded-xl text-[10px] font-black uppercase tracking-tight border ${areaCls}`}>
                            {u.area}
                          </span>
                    ...
                    <td className="px-6 py-5">
                      <div className="flex flex-col gap-1.5">
                        ...
                        <div className="flex items-center gap-2 text-[11px] font-black text-text hover:text-brand transition-colors cursor-pointer">
                          ...
                          <span className="truncate max-w-[140px] lowercase">{u.email}</span>
                        </div>
                      </div>
                    </td>
                    ...
                    {/* Status */}
                    <td className="px-6 py-5">
                      <span className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-[0.1em] border transition-all ${
                        u.status === "activo" ? "bg-emerald-50 text-emerald-600 border-emerald-100" : 
                        u.status === "suspendido" ? "bg-amber-50 text-amber-600 border-amber-100" :
                        "bg-red-50 text-red-600 border-red-100"
                      }`}>
                        <span className={`w-1.5 h-1.5 rounded-full animate-pulse ${
                          u.status === "activo" ? "bg-emerald-600" : 
                          u.status === "suspendido" ? "bg-amber-600" : "bg-red-600"
                        }`} />
                        {u.status}
                      </span>
                    </td>
                      ...
                      {openMenu === u.id && (
                        ...
                        >
                          <div className="px-3 py-2 mb-2 border-b border-border/60">
                             <p className="text-[10px] font-black text-text-muted uppercase tracking-[0.2em] m-0">Acciones Directas</p>
                          </div>
                          <button
                            onClick={() => onEdit(u)}
                          ...
                          >
                            {u.status === "activo" ? (
                              <><UserX size={14} className="text-amber-500" /> Suspender Acceso</>
                            ) : (
                              <><UserCheck size={14} className="text-emerald-500" /> Reactivar Acceso</>
                            )}
                          </button>
                          ...
                          >
                            <Trash2 size={14} className="group-hover/del:animate-bounce" /> Baja Definitiva
                          </button>
                        </div>
                      )}
                    </td>
