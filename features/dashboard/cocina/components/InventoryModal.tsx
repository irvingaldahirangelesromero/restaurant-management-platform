        ...
        <div className="p-8 pb-10 space-y-4 max-h-[60vh] overflow-y-auto">
          {inventario.map((item) => {
            ...
            const isCrit = item.stock <= item.minStock;
             ...
             return (
               <div key={item.name} className={`p-6 rounded-[28px] border transition-all duration-300 group hover:shadow-xl ${isCrit ? "bg-red-50/40 border-red-100" : "bg-surface border-border/60"}`}>
                  <div className="flex justify-between items-start mb-4">
                     <div>
                        <p className="font-display font-black text-[15px] text-text m-0 leading-tight mb-1.5 uppercase tracking-tight">{item.name}</p>
                        <div className="flex items-center gap-2">
                           <span className={`text-[10px] font-black uppercase tracking-widest ${isCrit ? "text-red-600" : "text-text-muted"}`}>
                              {item.stock} {item.unit} disponibles
                           </span>
                     ...
                     {isCrit && (
                        <button
                          onClick={() => enviarAlerta(item.name)}
