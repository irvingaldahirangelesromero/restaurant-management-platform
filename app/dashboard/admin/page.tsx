...
import { 
  ...
  Users, Settings, LogOut, TrendingUp, Clock, 
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    if (!stored) { redirectTo('/login'); return; }
    const parsed = JSON.parse(stored);
    if (parsed.roleName !== 'admin') { redirectTo('/dashboard'); return; }
    setUser(parsed);
  }, []);
    ...
    <div className="flex min-h-screen bg-[#0f0f0f] text-white font-sans">
      ...
      <aside className="w-64 border-r border-white/5 bg-[#0a0a0a] flex flex-col fixed h-full z-50">
        ...
        <nav className="flex-1 px-4 space-y-2 mt-4">
          ...
          <NavItem icon={<Users size={20}/>} label="Personal" onClick={() => redirectTo('/dashboard/admin/users')} />
          <div className="pt-4">
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-3">
              <div className="text-right">
                ...
                <p className="text-[10px] text-orange-500 uppercase font-black tracking-widest">Administrador</p>
              </div>
              <div className="w-12 h-12 bg-gradient-to-br from-orange-500 to-orange-700 rounded-2xl border border-white/10 shadow-lg shadow-orange-500/10 flex items-center justify-center font-black text-sm">
...
function NavItem({ icon, label, active = false, onClick }: { icon: React.ReactNode, label: string, active?: boolean, onClick?: () => void }) {
  return (
    <button onClick={onClick} className={`w-full flex items-center gap-4 px-4 py-3.5 rounded-2xl transition-all font-bold text-sm ${
      active ? 'bg-orange-600 text-white shadow-xl shadow-orange-600/20' : 'text-gray-500 hover:bg-white/5 hover:text-gray-200'
    }`}>
      <span>{icon}</span>
      {label}
    </button>
