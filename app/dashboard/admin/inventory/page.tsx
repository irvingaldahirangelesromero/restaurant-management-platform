...
> = {
  caducidad: { label: "Caducidad", color: "#dc2626", bg: "#fef2f2" },
  accidente: { label: "Accidente", color: "#d97706", bg: "#fffbeb" },
  calidad: { label: "Baja calidad", color: "#7c3aed", bg: "#faf5ff" },
  coccion: { label: "Merma cocción", color: "#0ea5e9", bg: "#f0f9ff" },
...
const MOCK_MERMAS: Merma[] = [
  {
    ...
    reason: "caducidad",
    justification:
      "Quedó fuera de refrigeración durante el corte de luz del martes.",
  ...
  },
  {
    ...
    reason: "calidad",
    justification:
      "Llegaron con moho del proveedor, se rechazaron 3kg de la entrega.",
...
export default function InventoryPage() {
  ...
  const [mermas, setMermas] = useState<Merma[]>(MOCK_MERMAS);
  const [tab, setTab] = useState<"productos" | "mermas">("productos");
  const [search, setSearch] = useState("");
  const [catFilter, setCatFilter] = useState<string>("all");
  const [statusFilter, setStatusFilter] = useState<string>("all");
  const [productModal, setProductModal] = useState<Product | null | "new">(
    null,
  );
  ...
  // Stats
  const criticals = products.filter(
    (p) => stockStatus(p) === "critical",
  ).length;
  ...
  function saveProduct(p: Product) {
    setProducts((ps) => {
      const ex = ps.find((x) => x.id === p.id);
      return ex ? ps.map((x) => (x.id === p.id ? p : x)) : [...ps, p];
    });
  }
  ...
  function adjustStock(id: number, newStock: number) {
    setProducts((ps) =>
      ps.map((p) =>
        p.id !== id
          ? p
